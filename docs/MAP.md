...
| `venice` | Venice | Italy | city | 🪙 gold | 🪵 timber, 🪨 marble | Y | T3 | **Venice** | Capital; the Arsenal. **HV(4)**. **+1 🪨/round** (balance retune). |
| `milan` | Milan | Italy | plains | 🪙 gold | 🪨 marble | N | T2 | Independent | Armorers & condottieri (Duchy of Milan). |
| `genoa` | Genoa | Italy | city | 🪙 gold | 🪨 marble | Y | T3 | **Genoa** | Capital; the Bank of St George. **HV(4)**. **+1 🪨/round** (balance retune). |
...
> **Balance retune (ratified):** `venice` and `genoa` each yield **+1 🪨 marble/round** — balance
> retune per the tuning report of PR #11 (@ `f760294`).
