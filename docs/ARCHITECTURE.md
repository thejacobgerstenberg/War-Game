...
* **Environment variables** (defaults live in code; unknown vars are ignored):
  ...
  | `GAME_SEED` | *(unset)* | ⚠ **TEST-ONLY.** When set to a 32-bit unsigned integer, every game started by the process uses it as the deterministic RNG seed instead of the crypto-random `generateGameSeed()` — whole games become reproducible (deck order, every roll). Resolved in `LobbyManager.startGame` (the engine stays pure and merely receives the seed). Defeats the fog-of-war secrecy of the deck ordering, so **never set in production**; a `test_knob_active` **warn** line is logged on every game start while active. Garbage values are ignored (warned, crypto-random fallback). Used by the Playwright E2E suite. |
  | `PRESTIGE_TARGET` | *(unset)* | ⚠ **TEST-ONLY.** When set to a positive integer, games are created with `GameState.prestigeTarget`, which `prestige.decideWinner` prefers over the ratified §13.2 `PRESTIGE_THRESHOLDS` — victory is still a REAL engine victory, checked only at Cleanup with all precedence/tiebreak rules intact, just at a lower bar (e.g. `PRESTIGE_TARGET=8` ends a 2-player E2E game in a few rounds). Resolved in `LobbyManager.startGame` and carried on state from creation (engine purity preserved). **Never set in production**; logs the same `test_knob_active` warn per game start; garbage values ignored with a warning. |
